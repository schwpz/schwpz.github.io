...
export async function buildAnalysis(events,{coupling,analogueDB,nowMs=Date.now()}={}){
  if(!coupling?.features) throw new Error('buildAnalysis: coupling FeatureCollection required');
  if(!analogueDB?.profiles) throw new Error('buildAnalysis: analogueDB required');
  const segments=[];
  let nearLocked5=0;
